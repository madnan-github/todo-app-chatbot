...
- **Procfile**: Create a `Procfile` at the root of the backend directory (or project root if applicable) with: `web: uvicorn src.main:app --host 0.0.0.0 --port $PORT`.
- **Environment**: Ensure logic for switching between `development` and `production` based on an `ENVIRONMENT` variable.
